...
export default function Home4() {
  ...
  useEffect(() => {
    const getUserCountry = async () => {
      try {
        // Using ipapi.co for IP geolocation (free, no API key required for basic usage)
...
console.log("Detected country:", countryCode);
        ...
        console.log(`User detected in country: ${countryCode}`);
      } catch (error) {
  ...
  /* ---------------- DATA RETRIEVAL WITH COUNTRY FILTER ---------------- */
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        // Map country code to country name for API
        ...
        const countryName = countryMap[userCountry] || userCountry.toLowerCase();
    ...
    return (
      <section className="w-full bg-white py-8 md:py-12 lg:pl-30">
        <div className="mx-auto max-w-8xl px-4 md:px-8">
          ...
          <div className="flex gap-4 md:gap-6 overflow-x-scroll no-scrollbar">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex-shrink-0 w-[90vw] md:w-[536px] h-auto md:h-[712px] rounded-2xl bg-gray-200 animate-pulse"></div>
            ))}
          </div>
        </div>
      </section>
          ...
          >
            {apiArticles.length > 0 ? (
              ...
              <div className="w-full py-12 text-center">
                <p className="text-gray-500">No articles found for your region. Please check back later.</p>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="p-6 md:p-8 lg:p-10">
                ...
                <div className="pt-6 mt-8 border-t">
                  <div className="flex justify-end">
                    {/* Button to open original article in new tab */}
                    <button
